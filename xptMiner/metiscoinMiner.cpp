...
static void
my_metis_core(sph_metis512_context *sc, const void *data, size_t len)
{
	sph_u32 p;
...
void metiscoin_process(minerMetiscoinBlock_t* block)
{
	...
	OpenCLKernel* kernel_update = OpenCLMain::getInstance().getDevice(0)->getContext()->getProgram(0)->getKernel("metis_update_g");
	...
	OpenCLBuffer* data = OpenCLMain::getInstance().getDevice(0)->getContext()->createBuffer(64, CL_MEM_READ_ONLY, NULL);
	...
	kernel_update->resetArgs();
	kernel_update->addGlobalArg(ctx);
	kernel_update->addGlobalArg(data);
	...
	for(uint32 n=0; n<0x1000; n++)
	{
		...
		for(uint32 f=0; f<0x8000; f++)
		{
			...
			sph_metis512_context ctx2;
			sph_metis512_init(&ctx2);
			my_metis_core(&ctx2, hash1, 64);
...
//
//
//			q->enqueueWriteBuffer(data, hash1, 64);
//			q->enqueueKernel1D(kernel_update, 1, 1);
//			q->enqueueWriteBuffer(ctx, &ctx2, 164);
//			q->finish();

			for (int i = 0; i < 36; i++) {
				if (ctx_metis.S[i] != ctx2.S[i]) {
					printf ("hashes are different %d %X %X\n", i, ctx_metis.S[i], ctx2.S[i]);
				}
			}
