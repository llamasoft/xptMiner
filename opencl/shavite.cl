...
#define AES_ROUND_LE(X, Y)   { \
		(Y.s0) = SHAVITE_LOOKUP0[(X.s0) & 0xFF]          \
			   ^ SHAVITE_LOOKUP1[((X.s1) >> 8) & 0xFF]   \
			   ^ SHAVITE_LOOKUP2[((X.s2) >> 16) & 0xFF]  \
			   ^ SHAVITE_LOOKUP3[((X.s3) >> 24) & 0xFF]; \
		(Y.s1) = SHAVITE_LOOKUP0[(X.s1) & 0xFF]          \
			   ^ SHAVITE_LOOKUP1[((X.s2) >> 8) & 0xFF]   \
			   ^ SHAVITE_LOOKUP2[((X.s3) >> 16) & 0xFF]  \
			   ^ SHAVITE_LOOKUP3[((X.s0) >> 24) & 0xFF]; \
		(Y.s2) = SHAVITE_LOOKUP0[(X.s2) & 0xFF]          \
			   ^ SHAVITE_LOOKUP1[((X.s3) >> 8) & 0xFF]   \
			   ^ SHAVITE_LOOKUP2[((X.s0) >> 16) & 0xFF]  \
			   ^ SHAVITE_LOOKUP3[((X.s1) >> 24) & 0xFF]; \
		(Y.s3) = SHAVITE_LOOKUP0[(X.s3) & 0xFF]          \
			   ^ SHAVITE_LOOKUP1[((X.s0) >> 8) & 0xFF]   \
			   ^ SHAVITE_LOOKUP2[((X.s1) >> 16) & 0xFF]  \
			   ^ SHAVITE_LOOKUP3[((X.s2) >> 24) & 0xFF]; \
	}
